...
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, hidden_size=1024, image_size=224, patch_size=14, num_channels=3, dtype=torch.float32):
        ...
        self.position_embedding = torch.nn.Embedding(self.num_positions, self.embed_dim, dtype=dtype)
...
class NoTPTransformerBlock(nn.Module):
    def __init__(self, cfg, layer_id: int, multiple_of=256, dtype=torch.float32):
        ...
        self.layer_norm1 = torch.nn.LayerNorm(
            cfg.hidden_size, eps=cfg.layernorm_epsilon, dtype=dtype
        )
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.net_2 = nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, dtype=dtype)
        self.net_3 = nn.Conv2d(512, 1024, kernel_size=3, stride=2, padding=1, dtype=dtype)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv2d(
            in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding, dtype=dtype
        )
