...
Usage:
    ...
    python OCRVL/evaluation/eval_ocrqwen3vl.py \\
        --benchmark scienceqa \\
    ...
    if model_type == "ocrqwen3vl":
        ...
        if render_questions:
            try:
                ...
                logger.error(f"  ✗ Vello renderer not available: {e}")
                logger.error("  Please build the renderer: cd Renderer && maturin develop --release")
                raise
...
Examples:
  ...
  python eval_ocrqwen3vl.py --benchmark scienceqa --output results/scienceqa.jsonl

  # Evaluate on M3CoT with LoRA
  python eval_ocrqwen3vl.py --benchmark m3cot --lora-path {default_lora} --output results/m3cot.jsonl

  # Quick test (10 samples)
  python eval_ocrqwen3vl.py --benchmark scienceqa --max-samples 10 --output results/test.jsonl
