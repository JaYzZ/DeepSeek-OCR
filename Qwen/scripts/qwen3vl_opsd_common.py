#!/usr/bin/env python3
"""Shared OPSD helpers for Qwen3-VL distillation scripts.

This module is intentionally kept as an internal utility surface for the public
OPSD trainers. It is not the supported training entrypoint.
"""
...
def _resolve_runtime_vae_checkpoint_path(config: dict[str, Any]) -> Path | None:
    env_checkpoint = str(os.environ.get("QWEN3VL_VAE_CHECKPOINT_PATH", "")).strip()
    if env_checkpoint:
    ...
    adapter_path = config.get("model", {}).get("adapter_name_or_path")
    if adapter_path:
        checkpoint_path = Path(adapter_path) / "vae.safetensors"
        if checkpoint_path.exists():
            return checkpoint_path

    init_lora_path = str(os.environ.get("INIT_LORA_PATH", "")).strip()
    if init_lora_path:
        checkpoint_path = Path(init_lora_path) / "vae.safetensors"
        if checkpoint_path.exists():
            return checkpoint_path

    vllm_lora_path = str(os.environ.get("VLLM_LORA_CHECKPOINT_PATH", "")).strip()
    if vllm_lora_path:
        checkpoint_path = Path(vllm_lora_path) / "vae.safetensors"
        if checkpoint_path.exists():
            return checkpoint_path
...
def _runtime_loss_spec() -> str:
    return str(os.environ.get("QWEN3VL_LOSS_TYPE", "ce+vae")).strip().lower()


def _runtime_vae_enabled(checkpoint_path: str | Path | None = None) -> bool:
    if checkpoint_path:
        checkpoint_file = Path(checkpoint_path)
        if checkpoint_file.is_dir():
            checkpoint_file = checkpoint_file / "vae.safetensors"
        if checkpoint_file.exists():
            return True

    explicit = os.environ.get("QWEN3VL_VAE_ENABLED")
    if explicit is not None and str(explicit).strip():
        return str(explicit).strip().lower() in {"1", "true", "yes", "on"}
    return "vae" in _runtime_loss_spec()


def _runtime_vae_trainable() -> bool:
    return str(os.environ.get("QWEN3VL_VAE_TRAINABLE", "1")).strip().lower() in {"1", "true", "yes", "on"}


def _runtime_vae_trainable_default(config: dict[str, Any], checkpoint_path: Path | None) -> bool:
    vae_cfg = config.get("vae") or {}
    if checkpoint_path is not None:
        return True
    if "trainable" in vae_cfg:
        return _parse_bool_flag(vae_cfg.get("trainable"))

    runtime_env = config.get("runtime_env_config")
    if runtime_env:
        runtime_cfg = _load_yaml(Path(runtime_env))
    else:
        runtime_cfg = {}
    return _parse_bool_flag(runtime_cfg.get("vae_trainable", False))


def _apply_vae_trainability(model, trainable: bool) -> None:
    vae = _resolve_latent_vae_module(model)
    if vae is None:
        return
...
def _ensure_runtime_vae_ready(
    ...
    *,
    ...
    if not _runtime_vae_enabled(checkpoint_file) and checkpoint_file is None:
        return False
    ...
    if checkpoint_file is not None:
        load_vae_checkpoint(model, str(checkpoint_file))
        os.environ["QWEN3VL_VAE_CHECKPOINT_PATH"] = str(checkpoint_file)

    _align_module_to_model_dtype_device(model, _resolve_latent_vae_module(model))
    _apply_vae_trainability(model, trainable)
    return True
...
def _prepare_model(config: dict[str, Any], accelerator: Accelerator):
    ...
    resolved_runtime_vae_checkpoint = _resolve_runtime_vae_checkpoint_path(config)
    ...
    if adapter_path:
        ...
        if resolved_runtime_vae_checkpoint is not None:
            resume_vae_expected = _ensure_resumed_vae_loaded(model, resolved_runtime_vae_checkpoint)
    ...
    _ensure_runtime_vae_ready(
        model,
        trainable=_runtime_vae_trainable(),
        checkpoint_path=resolved_runtime_vae_checkpoint,
    )
    ...
    if accelerator.is_main_process:
        ...
        if resume_vae_expected:
            print(f"[opsd] loaded resumed VAE from {resolved_runtime_vae_checkpoint}")

    require_vae_checkpoints = _runtime_vae_enabled(resolved_runtime_vae_checkpoint) or _resolve_latent_vae_module(model) is not None
    return model, model_dtype, require_vae_checkpoints
...
def _prepare_teacher_model(config: dict[str, Any], accelerator: Accelerator):
    ...
    runtime_vae_checkpoint = os.environ.get("QWEN3VL_VAE_CHECKPOINT_PATH", "").strip()
    teacher_vae_checkpoint = None
    if teacher_adapter_path and bool(teacher_cfg.get("load_vae_from_adapter", True)):
        teacher_vae_checkpoint = Path(teacher_adapter_path) / "vae.safetensors"
    elif runtime_vae_checkpoint:
        teacher_vae_checkpoint = runtime_vae_checkpoint
    _ensure_runtime_vae_ready(
        teacher_model,
        trainable=False,
        checkpoint_path=teacher_vae_checkpoint,
    )
...
def _prepare_runtime_env(config: dict[str, Any]) -> None:
    ...
    vae_cfg = config.get("vae") or {}
    resumed_vae_checkpoint = _resolve_runtime_vae_checkpoint_path(config)

    def _set_env_from_runtime(env_name: str, yaml_key: str, default: str) -> None:
        if env_name in os.environ and str(os.environ[env_name]).strip():
            return
        value = runtime_cfg.get(yaml_key, default)
        if value is None:
            value = default
        if isinstance(value, bool):
    ...
    if "QWEN3VL_VAE_ENABLED" not in os.environ or not str(os.environ["QWEN3VL_VAE_ENABLED"]).strip():
        if resumed_vae_checkpoint is not None:
            os.environ["QWEN3VL_VAE_ENABLED"] = "1"
            os.environ["QWEN3VL_VAE_CHECKPOINT_PATH"] = str(resumed_vae_checkpoint)
        elif "enabled" in vae_cfg:
            os.environ["QWEN3VL_VAE_ENABLED"] = "1" if _parse_bool_flag(vae_cfg.get("enabled")) else "0"
        else:
            os.environ["QWEN3VL_VAE_ENABLED"] = "1" if _parse_bool_flag(runtime_cfg.get("vae_enabled", False)) else "0"
    if "QWEN3VL_VAE_TRAINABLE" not in os.environ or not str(os.environ["QWEN3VL_VAE_TRAINABLE"]).strip():
        os.environ["QWEN3VL_VAE_TRAINABLE"] = (
            "1" if _runtime_vae_trainable_default(config, resumed_vae_checkpoint) else "0"
        )
    if "QWEN3VL_VAE_INTERMEDIATE_SIZE" not in os.environ or not str(os.environ["QWEN3VL_VAE_INTERMEDIATE_SIZE"]).strip():
        raw_intermediate = vae_cfg.get("intermediate_size", runtime_cfg.get("vae_intermediate_size", 512))
        try:
            os.environ["QWEN3VL_VAE_INTERMEDIATE_SIZE"] = str(int(raw_intermediate))
        except Exception:
            os.environ["QWEN3VL_VAE_INTERMEDIATE_SIZE"] = "512"
    if "QWEN3VL_MAX_NEW_TOKENS" not in os.environ or not str(os.environ["QWEN3VL_MAX_NEW_TOKENS"]).strip():
        raw_generation_max = generation_cfg.get("max_new_tokens", 40960)
        try:
            os.environ["QWEN3VL_MAX_NEW_TOKENS"] = str(int(raw_generation_max))
        except Exception:
            os.environ["QWEN3VL_MAX_NEW_TOKENS"] = "40960"
    if "MIN_CONTINUOUS_STEPS" not in os.environ or not str(os.environ["MIN_CONTINUOUS_STEPS"]).strip():
        raw_min_steps = generation_cfg.get("min_continuous_steps", 0)
        try:
            os.environ["MIN_CONTINUOUS_STEPS"] = str(max(0, int(raw_min_steps)))
        except Exception:
            os.environ["MIN_CONTINUOUS_STEPS"] = "0"
    if "MAX_CONTINUOUS_STEPS" not in os.environ or not str(os.environ["MAX_CONTINUOUS_STEPS"]).strip():
        raw_max = generation_cfg.get("max_continuous_steps")
        raw_tokens = generation_cfg.get("max_new_tokens", 40960)
        try:
...
def _init_vllm_rollout_engine(
    *,
    ...
    max_num_seqs = int(rollout_cfg.get("max_num_seqs", config["training"]["per_device_train_batch_size"]))
    ...
    llm_kwargs = {
        ...
        "enable_sleep_mode": sleep_enabled,
    ...
    if rollout_cfg.get("max_num_batched_tokens") is not None:
        llm_kwargs["max_num_batched_tokens"] = int(rollout_cfg["max_num_batched_tokens"])
    if rollout_cfg.get("enable_chunked_prefill") is not None:
        llm_kwargs["enable_chunked_prefill"] = bool(rollout_cfg["enable_chunked_prefill"])
...
def _generate_completions_with_vllm(
    *,
    vllm_engine,
    sampling_params,
    processor,
    metadata: list[dict[str, Any]],
    completion_dtype: torch.dtype,
    blocked_rollout_token_ids: set[int],
) -> list[torch.Tensor]:
    sleep_enabled = bool(getattr(vllm_engine, "opsd_enable_sleep_mode", False))
    if sleep_enabled and hasattr(vllm_engine, "wake_up"):
    ...
    removed_blocked_tokens = 0
    for output in outputs:
        completion = torch.tensor(output.outputs[0].token_ids, device="cpu", dtype=completion_dtype)
        completion, removed = _sanitize_rollout_completion_ids(completion, blocked_rollout_token_ids)
        removed_blocked_tokens += removed
...
if __name__ == "__main__":
    raise SystemExit("Use Qwen/scripts/train_qwen3vl_opsd.py instead.")
