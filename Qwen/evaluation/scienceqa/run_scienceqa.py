...
from Qwen.evaluation.scienceqa.dataset_utils import deterministic_limit, dump_image, load_dataset
from Qwen.evaluation.scienceqa.eval_utils import evaluate_records, save_eval_summary
...
def _prompt_style_from_env() -> str:
    return os.environ.get("SCIENCEQA_PROMPT_STYLE", "cot").strip().lower()


def build_scienceqa_prompt(line, dump_image_func, dataset_name):
    del dataset_name
    ...
    lecture = str(line.get("lecture", "") or "").strip()
    ...
    if prompt_style == "cot":
        prompt_parts.append("Think step by step, then answer with the final option letter in parentheses.")
    else:
        prompt_parts.append("Answer with the final option letter in parentheses.")
    prompt = "\n".join(prompt_parts)
...
def _load_rows(args):
    if args.data_dir:
        os.environ["SCIENCEQA_DATA_ROOT"] = args.data_dir
    data = load_dataset(args.dataset)
    return deterministic_limit(data, args.num_samples)


def _build_messages(data: pd.DataFrame, dataset_name: str):
    img_root = os.path.join(os.environ.get("LMUData", str(Path(__file__).parent.parent / "data")), "images", "ScienceQA")
    ...
    for _, line in tqdm(data.iterrows(), total=len(data), desc="Building prompts"):
        ...
        all_messages.append(build_scienceqa_prompt(line_dict, dump_image_func, dataset_name))
...
def run_inference(args):
    ...
    print(f"✓ ScienceQA inference completed in {elapsed:.2f}s")
...
def main():
    parser = argparse.ArgumentParser(description="ScienceQA benchmark runner")
    subparsers = parser.add_subparsers(dest="command", help="Command to run")

    infer_parser = subparsers.add_parser("infer", help="Run inference")
    infer_parser.add_argument("--model-path", type=str, default=str(QWEN3_VL_2B_THINKING))
    infer_parser.add_argument("--dataset", type=str, default="ScienceQA")
    ...
    infer_parser.add_argument("--presence-penalty", type=float, default=0.0)
    ...
    eval_parser.add_argument("--dataset", type=str, default="ScienceQA")
    ...
    if getattr(args, "prompt_style", None):
        os.environ["SCIENCEQA_PROMPT_STYLE"] = args.prompt_style
